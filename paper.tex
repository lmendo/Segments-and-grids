...
Given the coordinates $(x_1, y_1)$ of the first segment endpoint, with $0 \leq x_1 \leq a$, $0 \leq y_1 \leq b$, the second endpoint $(x_2, y_2)$ lies on a circle with radius $\len$ centered at $(x_1,y_1)$, as shown in Figure~\ref{fig: Pr_i}. The segment orientation is a random angle $\orient$ uniformly distributed on $[0,2\pi)$. It is clear from the figure that $i \geq n$ if and only if $x_2 \geq a(n-1)$ or $x_2 \leq -a(n-2)$; and these events are exclusive for $n \geq 2$. Thus
\begin{equation}
\label{eq: Pr i cond}
\Pr[i \geq n \cond x_1] = \Pr[x_2 \geq a(n-1) \cond x_1] + \Pr[x_2 \leq -a(n-2) \cond x_1].
\end{equation}
The two conditional probabilities on the right-hand side of \eqref{eq: Pr i cond} are different in general. However, averaging over $x_1$ gives, by symmetry, $\Pr[x_2 \geq a(n-1)] = \Pr[x_2 \leq -a(n-2)]$; and consequently, for $n \geq 2$,
\begin{equation}
\label{eq: Pr i = 2 Pr}
...
Consider the event $x_2 \geq a(n-1)$ conditioned on $x_1$, with $n \geq 2$. There are three possibilities depending on $x_1$, $n$ and $\len$. If $a(n-1) < \len$, regardless of $x_1$ the length $\len$ is enough for $x_2$ to exceed $a(n-1)$ for some angles $\orient$. This is depicted in Figure~\ref{fig: Pr_i_full}, where the section of the arc with solid line represents those angles for which $x_2 > a(n-1)$. If $\len \leq a(n-1) < \len+a$, the length can only be enough provided that $x_1 \geq a(n-1)-\len$, which corresponds to the shaded region in Figure~\ref{fig: Pr_i_notfull}. Lastly, if $\len+a \leq a(n-1)$ it is not possible for $x_2$ to exceed $a(n-1)$. Note that the coordinate $y_1$ is irrelevant to this.
...
In view of Theorem~\ref{theo: funta, form}, the average number of touches tiles has a remarkably simple dependence on the segment length, namely an affine function. Conversely, for any $t>1$ it is immediate to compute the length of a random segment that touches $t$ tiles on average, given as $\funta^{-1}(t)$.

The results in this section make clear the relationship between the problem considered here (average number of squares touched by a random segment of fixes length) and the Buffon's needle problem (probability that a random segment of fixed length crosses a line of a regular structure of parallel strips) \cite[section 1.1]{Mathai99}. Firstly, since $\pi$ is involved in \eqref{eq: funta, form}, it is possible to design a simple probabilistic experiment to estimate the value of $\pi$, as in Buffon's original experiment. For example, choosing $a=b=\len=1$ gives $\funta(\len) = 1+1/\pi$.
...
Lastly, $\Pr[i \geq n]$ as computed in Proposition~\ref{eq: prop Pr i} for $n \geq 2$ can be seen as a generalization of the probability of crossing at least one line in Buffon's experiment, when a needle endpoint is only allowed to move in a narrower region of width $n-1$ times smaller than the width of the strips (instead of moving across the full strip as in Buffon's setting).
