...
The question of determining how many tiles a segment of a given length touches on average will be referred to as the \emph{average-number} problem. As will be seen, solving this also answers the inverse question (segment length to touch a given number of tiles on average), which thus need not be considered separately. By symmetry, one endpoint of the segment can be assumed to lie in a fixed, reference tile. The position of this endpoint will be considered to be \emph{uniformly} distributed on the tile. The segment orientation will be have a \emph{uniform} distribution on the set of all possible directions, and is \emph{independent} of the endpoint position.
...
The problems addressed in this paper are related to \ldots ***maybe a short paragraph here mentioning related problems: Buffon's needle, Buffon-Laplace problem. Some references: ``Buffon's Noodle Problem'', J.F.~Ramaley, 1969. ``On Laplace's Extension of the Buffon Needle Problem'', B.J.~Arnow, 1994. \\
\url{https://www.jstor.org/stable/2317945} \\
\url{https://www.jstor.org/stable/2687085}

The rest of the paper is organized as follows. \S\ref{part: fund results} presents fundamental results, which form the basis of the subsequent analysis. The direct and inverse problems are considered in \S\ref{part: max}, first for real-valued lengths, and then for integer-valued lengths on a unit square grid. The average-number problem is analyzed in \S\ref{part: ave}.
...
\section{Maximum number of tiles: direct and inverse problems}
\label{part: max}
...
\section{Average number of tiles}
\label{part: ave}

***A theorem, and maybe one or two propositions. It is interesting to state $\Pr[i \geq i_0]$ (averaged over $x$) in a proposition, as it can be compared with the Buffon experiment.

***Define the average number of tiles touched as a function $\funta: \mathbb R^+ \to \mathbb R^+$.

***The formula for the average number of files is:
\begin{equation}
...
***Relation the the problem of the Buffon needle. On one hand, the formula for b=infinity gives the average number of lines crossed in the Buffon experiment. On the other hand, the intermediate values $\Pr[i \geq i_0]$ (proposition) are a generalization of the probability of crossing at least a line in the Buffon experiment, when the needle endpoint is only allowed to move in a smaller vertical strip.
...
***Maybe a graph comparing maximum and average. Or compute/plot their ratio. The asymptotic slopes are $\sqrt(1/a^2+1/b^2)$ for $\funt$ and $2/\pi(1/a+1/b)$. It may be interesting to consider the ratio of asymptotic slopes. Some quick plot of that ratio suggests it is maximum for $a=b$, and equals $sqrt(8)/pi = 0.9003$.

***The distribution would be interesting, not just the average. But it is more difficult to obtain, because of the statistical relationship between the random variables i and j. This relationship does not affect the \emph{average} of the random variable $i+j-1$ (for a given length), but it does affect the its \emph{distribution}. [I think it is interesting to mention this].
