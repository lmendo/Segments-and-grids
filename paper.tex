...
Consider the event $x_2 \geq a(n-1)$. There are three possibilities depending on $x_1$, $n$ and $\len$. If $a(n-1) < \len$, the length $\len$ is enough for $x_2$ to exceed $a(n-1)$ for some angles $\orient$, regardless of $x_1$. This is depicted in Figure~\ref{fig: Pr_i_full}, where the section of the arc with solid line represents those angles for which $x_2 > a(n-1)$. If $\len \leq a(n-1) < \len+a$, the length is enough only if $x_1 \geq a(n-1)-\len$, which corresponds to the shaded region in Figure~\ref{fig: Pr_i_notfull}. Lastly, if $\len+a \leq a(n-1)$ it is impossible that $x_2$ exceeds $a(n-1)$. Note that the value $y_1$ is irrelevant for this.
...
\end{equation}
gives the average number of lines crossed in Buffon's experiment. For $\len \leq a$ the segment can only cross one line, and \eqref{eq: number of lines crossed, Buffon} coincides with the probability of crossing \cite[section 1.1]{Mathai99}.
