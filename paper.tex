...
\end{equation}
The uniform distribution of the corner of the shaded rectangle on the chord is equivalent to a uniform distribution of its $x$ coordinate on the projection of the chord onto the horizontal axis. Denoting the slope of the chord by $m_{\tiles,n}$, the projection has length
\[
\frac{\tilde c_{\tiles,n}} {\sqrt{1 + m_{\tiles,n}^2}},
\]
and $\tilde\fdp(\theta)$ is obtained, using the transformation theorem for continuous random variables \cite[section~5.2]{Papoulis91}, as
\begin{equation}
\label{eq: tilde fdp theta}
\tilde \fdp(\theta) = \frac {\sqrt{1 + m_{\tiles,n}^2}} {\tilde c_{\tiles,n} |\diff \theta / \diff x|}
\end{equation}
...
\label{eq: tilde pdf pdf bound <}
\frac {\tilde \fdp(\theta)} {\fdp(\theta)} &< \left(\frac{\tiles-3/2}{\tiles-3}\right)^2 = \left(1+\frac{3/2}{\tiles-3}\right)^2 \leq 1 + \frac {21/4} {\tiles-3}.
\end{align}
