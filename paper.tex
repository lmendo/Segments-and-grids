...
\end{equation}
which can have zero, one or two real-valued solutions. These cases respectively mean that the solid line in Figure~\ref{fig: ijLS_1p35_detail} is exterior, tangent or secant to the circle (the figure shows the latter case). Only the solution with $\isolr, \jsolr \geq 2$, if any, is of interest. This solution can only exist as the one with largest $\isolr$ value when there are real-valued solutions, and it is obtained as %***Here rewriting. Maybe rephrase the following: the expression is general (isn't it?), i.e. may give complex values; and the three cases (0, 1 or 2 real solutions are not clearly identified) 
\begin{equation}
...
If \eqref{eq: quadratic, i} has two real-valued solutions or one real-valued double solution, which occurs when $\len \geq \sqrt{a^2+b^2}/2$, the largest solution will have $\jsolr < 2$ if $\isolr < (b^2/a^2+5)/2$, as can be seen setting $\jsolr=2$ in \eqref{eq: icont jcont 2}; or equivalently if $\sqrt{a^2+b^2}/2 \leq \len < (a^2+b^2) / (2a)$, as can be seen substituting $\jsolr < 2$ and $\isolr < (b^2/a^2+5)/2$ in \eqref{eq: icont jcont 1}. Since $\isolr < (b^2/a^2+5)/2 < 3$ for $a \geq b$, only pairs with $i=2$ are achievable in this case again, and thus the solution $\isoli$ should also be $2$.
