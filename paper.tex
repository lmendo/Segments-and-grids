...
\begin{abstract} % ***Alex: please check / revise 
The relationship between the length of a line segment and the number of tiles it visits on a rectangular grid is addressed in this paper. The square grid is also considered explicitly, as some of the specific problems studied are more tractable in that particular case. The segment position and orientation are modelled as either deterministic or random. In the former setting, the maximum number of visited tiles given the segment length and the infimum length for a desired number of visited tiles are characterized. For a segment of fixed length with random position and orientation, the average number of visited tiles and the probability of visiting the maximum number of tiles on a square grid are studied. These questions are related to Buffon's needle problem and its extension by Laplace.

\emph{Keywords:} Discrete geometry, Geometric probability, Rectangular lattice, Buffon's needle problem.

\emph{MSC2020:} 52C99, 60D05. % ***Alex: is there a better one?
...
\includegraphics[height=.235\textheight]{pairs_bounds_1}% *!*
}\hfill%
\subfigure[$a = 1.35$, $b = 1$]{%
\label{fig: pairs_bounds_1p35}%
\includegraphics[height=.235\textheight]{pairs_bounds_1p35}% *!*
}\hfill%
\subfigure[$a = \sqrt{2}$, $b = 1$]{%
\label{fig: pairs_bounds_sqrt2}%
\includegraphics[height=.235\textheight]{pairs_bounds_sqrt2}% *!*
}%
