...
% ***LM: I would make the rational-length extension more explicit, and invoke a suitable theorem (with the corresponding book reference) for the real-length case. Or better yet: the first reference linked as a comment in the introduction (https://www.jstor.org/stable/2317945?seq=1#metadata_info_tab_contents) already gives the $2\len/(\pi a)$ formula. We can directly start from that formula, referencing that paper.
The classic Buffon's Needle result (**AA cite Buffon's 1777 paper?** **LM I think maybe something more modern such as the references that are linked in the Introduction section**) states that if a needle of length $1$ is tossed with uniformly random position and orientation on
infinite ruled paper with parallel lines a distance of $1$ apart the probability that it crosses any line is $2/\pi$.

This is also the expected (that is, average) number of crossings, since the needle cannot reach more than one line, ignoring the event of it lying exactly along a line, which has probability $0$. For a needle of length $2$, the expected number of crossings is double the amount of $2/\pi$, since expectation is additive and thus unchanged if the needle is broken into two length-$1$ halves that are tossed separately, and their total crossings are counted. Conversely, for a needle of length $1/2$ the expectation would be half the original one.

A similar argument extends this proportionality to all rational lengths, and by monotonicity all real lengths. Thus for a needle of length $\len$, the expected number of crossings is $2\len/\pi$. If the lines are a distance $a$ apart, this is equivalent to shrinking the needle by a factor of $a$, giving $2\len/(\pi a)$ as expected number of crossings.

Consider a rectangular grid with horizontal spacing $a$ and vertical spacing $b$. Equivalently, this is paper ruled with horizontal lines $b$ apart and vertical lines $a$ apart. The distribution of the number of crossings of a needle tossed onto this grid is known as the Buffon-Laplace Needle Problem (*** cite). This number decomposes into the counts of crossings of horizontal and vertical grid lines, and thus the expected number of crossings is the sum of those for lined paper of width $a$ and $b$ respectively:
\[
