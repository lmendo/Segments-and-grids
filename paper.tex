...
\begin{equation}
\label{eq: probmax} %***Change order of the two summands under each square root here and in the rest of the proof, in accordance with \eqref{eq: square, odd max tiles, lengths}. (Or change there, but I think that is less preferable because it would correspond to using the y coordinate, then the x coordinate from the figure)
\begin{split}
...
\caption{Segment orientations and positions of the first endpoint within the reference tile for a given $\tiles = \funt(\len)$, with $\tiles$ odd. Example for $\tiles = 7$
}%
...
\caption{Tiles that can contain the second endpoint of a segment that visits the maximum number of tiles $\tiles= \funt(\len)$, with $\tiles$ odd. Example for $\tiles = 5$%
}%
