...
\title{
On the maximum number of tiles touched\\
by a line segment on a rectangular grid
}
...
The definition uses the interior of the tile, excluding the border, to avoid uninteresting results such as a ``zero-length'' segment touching (a vertex of) $4$ tiles.}
What is the maximum number of tiles that the segment can touch?

As an example, consider $a=1.35$, $b=1$. A segment of unit length can be placed as shown in Figure~\ref{fig: examples} (left) to touch $3$ tiles. In fact, the solution for $\len=1$ is $3$ tiles. The figure also illustrates that the solution for length $2.4$ is $5$ (center), and for $4.7$ it is $8$ (right).
...
The \emph{inverse} problem is, given $\tiles \in \mathbb N$, to determine the infimum length of all segments that touch at least $\tiles$ tiles. For real-valued lengths this infimum is not a minimum, because given a segment that touches $\tiles$ tiles, its length can always be reduced by some non-zero amount without changing the number of touched tiles. This is a consequence of the interior of the each tile being an open set.
...
These problems are related to \ldots ***maybe a short paragraph here mentioning related problems: Buffon's needle, Buffon-Laplace problem. Some references: ``Buffon's Noodle Problem'', J.F.~Ramaley, 1969. ``On Laplace's Extension of the Buffon Needle Problem'', B.J.~Arnow, 1994. \\
\url{https://www.jstor.org/stable/2317945} \\
\url{https://www.jstor.org/stable/2687085}

This paper analyzes both problems for real-valued lengths, and for integer-valued lengths on a unit square grid. Section~\ref{part: fund results} presents fundamental results, from which the characterization is obtained in Section~\ref{part: arbitrary grid, real lengths} for an arbitrary grid with real-valued lengths; in Section~\ref{part: square grid, real lengths} for a square grid with real-valued lengths; and in Section~\ref{part: unit square grid, integer lengths} for a unit square grid with integer-valued lengths.
...
For $a=b$, either from symmetry considerations or particularizing the formulas in this section it stems that the optimal orientation of the segment is close to $45^\circ$. This case will be dealt with in Section~\ref{part: square grid, real lengths}, as it lends itself to somewhat simplified formulas.
...
\label{part: square grid, real lengths}

Particularizing the results in Section~\ref{part: arbitrary grid, real lengths} to a square grid, $a=b$, obviously yields simpler formulas.
...
In order to prove that $\funti(\leni+1)-\funti(\leni) \in \{1,2\}$, consider the function $f(x) = \sqrt{2x^2-2}$ for $x \in \mathbb R$, $x>1$. Its first derivative is
\begin{equation}
f'(x) = \frac {\sqrt{2} \, x} {\sqrt{x^2-1}},
\end{equation}
and its second derivative is easily seen to be negative. Therefore $f'(x)$ can be bounded for $x \geq 3$ as
\begin{equation}
\label{eq: der bounds}
\lim_{x \rightarrow \infty} f'(x) = \sqrt{2} < f'(x) < f'(3) = 3/2.
\end{equation}
For $\leni \geq 2$, by the mean value theorem \cite[section~5.3]{Abbott15}, when $\leni$ is increased to $\leni+1$ the term $\sqrt{2\leni^2-2}$ in \eqref{eq: funti leni} has an increment that equals $f'(x)$ for some $\leni < x < \leni+1$. Therefore
\begin{equation}
