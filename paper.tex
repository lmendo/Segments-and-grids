...
% *** AA: Maybe add a transition that now we're switching to looking at the average number of tiles visited by a random segment.
...
To show \eqref{eq: theo: funt, sq, form; funt, simpler}, we refer back to \eqref{eq: theo: funt, sq, form; funl} in Corollary~\ref{theo: funl, sq, form} for $\tiles \geq 3$:
\begin{equation}
\funl(\tiles) = \sqrt{\left\lceil \frac{(\tiles-3)^2} {2} \right\rceil}
\end{equation}
Then, the pseudo-inverse function $\funt(\len)$ is given by the largest positive integer $t$ such that 
\begin{equation}
\left\lceil \frac{(\tiles-3)^2} {2} \right\rceil < \len ^ 2
\end{equation}
Since the left-hand side is an integer, the condition of being strictly less than $\len^2$ is equivalent to: 
\begin{equation}
\left\lceil \frac{(\tiles-3)^2} {2} \right\rceil  \leq \left\lceil \len^2 \right\rceil  - 1 
\end{equation}
and removing the now-superfluous ceiling on the left side gives:
\begin{equation}
\frac{(\tiles-3)^2} {2}\leq  \left\lceil \len^2 \right\rceil  - 1 
\end{equation}
Finally, we solve for $\tiles$:
\begin{equation}
\tiles \leq \sqrt{2 \left\lceil \len^2 \right\rceil -2 } + 3
\end{equation}
The desired quantity $\funt(\len)$, that is the largest positive integer $\tiles$ satisfying the above, is thus the floor of the right-hand side as given by \eqref{eq: theo: funt, sq, form; funt, simpler}.
\end{proof}
...
\begin{proof}
% *** Is this writeup too informal/non-rigorous?
The classic Buffon's Needle result (*** cite Buffon's 1777 paper?) says that if we take infinite ruled paper with parallel lines a distance of $1$ apart, and we toss a needle of length $1$ with uniformly random position and orientation, the probability that it crosses any line is $2/\pi$.

This is also the expected (that is, average) number of crossings, since this needle cannot reach more than one line\footnote{We may ignore the probability-zero event of it lying exactly along a line.}. For a needle of length $2$, the expected number of crossings is double the amount of $2/\pi$, since expectation is additive and so unchanged if we instead break this needle into two length-$1$ halves
that we toss separately and count their total crossings. Conversely, for a needle of length $1/2$, it would be half the original expectation.

A similar argument extends this proportionality to all rational lengths, and by monotonicity all real lengths: for a needle of length $l$, the expected number of crossings is
$\frac{2\len}{\pi}$.
If the lines are a distance $a$ apart, this is equivalent to shrinking the needle by a factor of $a$, giving expected crossings
$\frac{2\len}{\pi a}$.

Now, consider a rectangular grid paper with parameters of width $a$ and height $b$. Equivalently, this is paper ruled with horizontal lines $b$ apart and vertical lines $a$ apart. The distribution of the number of crossings of a needle tossed onto this grid is known as the Buffon-Laplace Needle Problem (*** cite). This number decomposes into the counts of crossings of horizontal and vertical grid lines, so the expected number of crossings is the sum of those for lined paper of width $a$ and $b$ respectively:
\begin{equation}
\frac{2\len}{\pi}\left(\frac \len a + \frac \len b\right)
\end{equation}
As noted in Proposition \ref{prop: i+j-1}, the number of grid cells visited by a segment is the count of its grid line crossings plus one -- unless, that is, it exactly crosses any grid points, but that occurs with probability zero and so doesn't affect the expectation. So, $\funta(\len)$ is the above quantity plus one.
