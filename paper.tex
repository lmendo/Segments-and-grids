...
\end{equation}
or equivalently, defining $\nextrat = \sqrt{\len^2/2-(\tiles-3)^2/4}$,
\begin{equation}
...
2 \sum_{k=1}^{\lceil\nextrat-1 \rceil} g\left(\len, \frac{\tiles-3} 2 - k, \frac{\tiles-3} 2 + k \right) &\text{for $\tiles$ odd} \\[6mm] % *!*
\displaystyle
2 g\left(\len, \frac{\tiles-4} 2, \frac{\tiles-2} 2 \right) +
2 \sum_{k=1}^{\lceil\nextrat-3/2 \rceil} g\left(\len, \frac{\tiles-4} 2 - k, \frac{\tiles-2} 2 + k \right) &\text{for $\tiles$ even}.
\end{cases}
\end{equation}
\end{theorem}
...
Given $\tiles$, the range of lengths $\len$ such that $\tiles = \funt(\len)$ is either \eqref{eq: square, odd max tiles, lengths} or \eqref{eq: square, even max tiles, lengths}. For $\len$ in this range, the number of tiles with lower-left corners $(i-1,j-1)$, $i,j \geq 2$, $i+j-1=\tiles$ that can be reached, i.e.~the number of terms in \eqref{eq: probmax impl}, depends on $\len$. If $\tiles$ is odd the minimum number is $1$, corresponding to a tile with lower-left corner on the line $x=y$, which gives the first term in the first expression in \eqref{eq: probmax expl}. For $\tiles$ even the minimum number is $2$, corresponding to two symmetrical tiles whose $x$ and $y$ coordinates differ by $1$, which give the first term in the second expression. Either for $\tiles$ odd or even, additional tiles may become reachable as $\len$ grows (within the range allowed by $\tiles$), and these always occur in symmetrical pairs. These tiles (if any) correspond to the terms in the (possibly empty) sum indexed by $k$ in either expression in \eqref{eq: probmax expl}. 
...
As can be seen in Figure~\ref{fig: probmax_terms}, for each $\tiles$ there is a maximum number of tiles that can be reached, beyond which incrementing $\len$ only results in $\tiles$ increasing by $1$. For a given $\len$, the number of reachable tiles can be determined by counting how many markers are covered by the length $w$ defined in Figure~\ref{fig: probmax_nterms}. This length satisfies the relationship $w^2 = \len^2 - (\tiles-3)^2/2$, and thus $K$ as defined in the theorem equals $w/\sqrt{2}$. From the figure, the number of terms in the sum over $k$ (empty circular markers) is $\lceil w/\sqrt 2 - 1 \rceil$ or $\lceil w/\sqrt 2 - 3/2 \rceil$ for $\tiles$ odd or even, i.e.~$\lceil \nextrat - 1 \rceil$ or $\lceil \nextrat - 3/2 \rceil$ respectively. This establishes \eqref{eq: probmax expl}.
\begin{figure}
\centering%
\subfigure[Odd $\tiles$]{%
...
The probability $\probmax(\len)$ of visiting the maximum number of tiles on a unit square grid, computed from Theorem~\ref{theo: probmax, sq}, is shown in Figure~\ref{fig: probmax_len} shows. As $\len$ grows, $\probmax(\len)$ has a jump when $\funt(\len)$ increases by $1$ (and there $\probmax(\len)$ takes the value approached from the left). This happens when $\len$ equals the right endpoint of the interval \eqref{eq: square, odd max tiles, lengths} for $\tiles$ odd or of the interval \eqref{eq: square, even max tiles, lengths} for $\tiles$ even. These length values will be denoted as $\len_\tiles$:
\begin{equation}
\label{eq: len tiles}
\len_\tiles = \begin{cases}
\displaystyle
\frac{\sqrt{(\tiles-3)^2 + (\tiles-1)^2}} {2} = \sqrt{\frac{(\tiles-2)^2+1}{2}} & \text{ for $\tiles$ odd, $\tiles \geq 3$} \\
\displaystyle
...
For the purpose of establishing \eqref{eq: probmax lim sqrt tiles}, consider the graph in Figure~\ref{fig: probmax_nterms} with $\len= \len_\tiles$. It is convenient to rotate this graph by $45^\circ$ and shift it so that the original $x+y = \tiles-1$ line (shown dashed in Figure~\ref{fig: probmax_nterms}) becomes the new horizontal axis. Figure~\ref{fig: probmax_asint_turned} shows the result, using $\tiles=9$ as an example. The circular markers in this figure correspond, in the original graph before rotating, to the lower-left corners $(i-1,j-1)$ of the tiles with $i+j-1=\tiles$, $(i-2)^2+(j-1)^2<\len_\tiles$. Each of these tiles can be (partially) seen above the corresponding marker in Figure~\ref{fig: probmax_asint_turned}, its sides forming $45^\circ$ degrees with respect to the horizontal axis. Let $x_{\tiles, n}$ denote the coordinates of these points along the horizontal axis of Figure~\ref{fig: probmax_asint_turned}.

Using the coordinate axes in Figure~\ref{fig: probmax_asint_turned}, the circular arc satisfies the equation $y = \funcirc(x)$ with
\begin{equation}
\label{eq: funcirc}
\funcirc(x) = h_\tiles - \len_\tiles + \sqrt{\len_\tiles^2-x^2}.
\end{equation}
The $y$ coordinate of the arc for $x=x_{\tiles,n}$ will be denoted as $y_{\tiles,n}$. The half-width of the arc along the horizontal axis, $w_\tiles$, is computed in the proof of Theorem~\ref{theo: probmax, sq} (or from the right triangle in Figure~\ref{fig: probmax_asint_turned}), and using \eqref{eq: len tiles} it can be expressed as
\begin{equation}
\label{eq: w}
...
\sqrt{1+\genvar} \leq 1 + \frac \genvar 2
\end{equation}
implies that
\begin{equation}
\label{eq: asympt w}
...
\frac{\tiles-2}{\sqrt{2}} \leq \len_\tiles \leq \frac{\tiles-2}{\sqrt{2}} + \frac{1}{2\sqrt{2}(\tiles-2)}
...
\end{equation}
can be bounded from \eqref{eq: asympt len tiles} as
\begin{equation}
\label{eq: h bound}
\frac 1 {\sqrt{2}} \leq h_\tiles \leq \frac 1 {\sqrt{2}} + \frac{1}{2\sqrt{2}(\tiles-2)}.
\end{equation}

\begin{figure}%
\centering
\includegraphics[width=.72\textwidth]{probmax_asint_turned}%
...
As argued in the proof of Theorem~\ref{theo: probmax, sq}, $\probmax(\len_\tiles)$ is the sum of several terms $\probmax_{i,j}(\len_\tiles)$, $i+j-1=\tiles$. Each term is associated with one of the tiles shown in Figure~\ref{fig: probmax_asint_turned} (or in Figure~\ref{fig: probmax_nterms}), and includes a factor of $4$ to account for the symmetrical tiles in the other quadrants. For this proof it is more convenient to compute $\tiles \probmax_{i,j}(\len_\tiles)$, and to change the indexing as follows:
\begin{equation}
P_{\tiles,n} = \tiles \probmax_{i,j}(\len_\tiles),
\end{equation}
where $n$ equals $1$ for the smallest $i$ in \eqref{eq: probmax impl}, increases with $i$, and has a maximum value $N$ equal to the number of summands in that expression. Thus
\begin{equation}
\label{eq: sum p t n}
\tiles \probmax(\len_\tiles) = \sum_{n=1}^N P_{\tiles,n}.
\end{equation}

Consider the tile depicted with thick line in Figure~\ref{fig: probmax_asint_turned}, for a generic value of the index $n$ in Figure~\ref{fig: probmax_asint_turned}, or of the indices $i,j$ in Figure~\ref{fig: probmax_nterms}. For this tile, $P_{\tiles,n}/t = \probmax_{i,j}(\len_\tiles)$ is obtained as the area of the shaded rectangle averaged over $\theta$ uniformly distributed on $[0,2\pi)$, and then multiplied by $4$, in the same way as in the proof of Theorem~\ref{theo: probmax, sq}. Equivalently, the average can be restricted to the range of values of $\theta$ for which the shaded area is non-zero, corresponding to the arc with thick line in the figure, of length $c_{\tiles,n}$, and the result multiplied by $c_{\tiles,n}/(2\pi\len_\tiles)$. Denoting the average over this restricted range by $A_{\tiles,n}$,
\begin{equation}
...
P_{\tiles,n}
&\geq \left(1 + \frac {3/2} {\tiles-3/2} \right) \frac{2 \sqrt{2} A_{\tiles,n} c_{\tiles,n}}{\pi} = \frac{2 \sqrt{2} A_{\tiles,n} c_{\tiles,n}}{\pi} + \frac{3/\sqrt{2}}{\tiles-3/2}.
\end{align}

To calculate $A_{\tiles,n}$ and $c_{\tiles,n}$ it is easier to replace the arc by its chord, as depicted in Figure~\ref{fig: probmax_asint_detail}. The approximation error for either $A_{\tiles,n}$ or $c_{\tiles,n}$ tends to $0$ as $\tiles \rightarrow \infty$. Specifically, regarding $A_{\tiles,n}$, the difference in $y$ between the arc and the chord is upper-bounded by the length of the chord multiplied by the maximum of the absolute value of the arc slope. The former is at most $\sqrt{2}$, and the latter is bounded using \eqref{eq: funcirc}, \eqref{eq: w} and \eqref{eq: asympt len tiles} as
\begin{equation}
\label{eq: asympt slope bound}
\left| \frac{\diff \funcirc(x)}{\diff x} \right| = \frac{x}{\sqrt{\len_\tiles^2-x^2}} \leq \frac{w_\tiles}{\len_\tiles} \leq \sqrt{\frac{2}{\tiles-2}}.
\end{equation}
Thus the difference in $y$ is at most $2/\sqrt{\tiles-2}$. This translates into a difference of at most  $\sqrt{2/(\tiles-2)}$ in each dimension of the shaded rectangle; or, since the maximum dimensions are $1$, with at least one of them being strictly less, a difference in area smaller than twice that value. Therefore the approximation error in $A_{\tiles,n}$ is less than $2\sqrt{2}/\sqrt{\tiles-2}$.

As for approximating $c_{\tiles,n}$ by the chord length, the latter is $2\len_\tiles \sin(c_{\tiles,n}/(2\len_\tiles))$. Taking into account that
\begin{equation}
\label{eq: sin asympt pos}
\sin\genvar > \genvar - \frac{\genvar^3} 6 \quad \text{for } s > 0
\end{equation}
and making use of \eqref{eq: c tiles n bound} and \eqref{eq: asympt len tiles}, this is bounded as
\begin{equation}
2\len_\tiles \sin \frac{c_{\tiles,n}}{2\len_\tiles} > c_{\tiles,n} -\frac{c_{\tiles,n}^2}{24\len_\tiles^2} \geq c_{\tiles,n} -\frac{\pi^2}{96\len_\tiles^2} \geq c_{\tiles,n} -\frac {\pi^2} {48(\tiles-2)^2},
\end{equation}
and thus the length difference is at most $\pi^2/(48(\tiles-2)^2)$.

\begin{figure}
\centering
\includegraphics[width=.7\textwidth]{probmax_asint_detail}%
\caption{Computation of $P_{\tiles,n}$}%
\label{fig: probmax_asint_detail}%
\end{figure}%

Let $\tilde A_{\tiles,n}$ and $\tilde c_{\tiles,n}$ denote the approximations of $A_{\tiles,n}$ and $c_{\tiles,n}$ obtained using the chord instead of the arc. From the preceding two paragraphs,
\begin{equation}
\label{eq: P i j asympt 3}
\tilde A_{\tiles,n} \tilde c_{\tiles,n} < A_{\tiles,n} c_{\tiles,n} <
\tilde A_{\tiles,n} \tilde c_{\tiles,n} + \frac{\pi^2 A_{\tiles,n}}{48(\tiles-2^2)} + \frac{2\sqrt{2} c_{\tiles,n}}{\sqrt{\tiles-2}}.
\end{equation}
Using \eqref{eq: A tiles n bound} and \eqref{eq: c tiles n bound} into \eqref{eq: P i j asympt 3} and combining with \eqref{eq: P i j asympt 2 1} and \eqref{eq: P i j asympt 2 2},
\begin{equation}
\label{eq: P i j asympt 3 bis}
P_{\tiles,n} = \frac{2\sqrt{2} \tilde A_{\tiles,n} \tilde c_{\tiles,n}}{\pi} + E_{\tiles, n}
\end{equation}
with $|E_{\tiles, n}| < e(\tiles)$, where $e$ is a function that depends only on $\tiles$ and tends to $0$ as $\tiles \rightarrow 0$.

Denoting the slope of the chord by $m_{\tiles,n}$, it is clear from Figure~\ref{fig: probmax_asint_detail} that
\begin{equation}
\frac{y_{\tiles,n}-a_{\tiles,n}/\sqrt{2}}{a_{\tiles,n}/\sqrt{2}} = m_{\tiles,n},
\end{equation}
and therefore $a_{\tiles,n} = \sqrt{2}y_{\tiles,n}/(1+m_{\tiles,n})$. Similarly, $b_{\tiles,n} = \sqrt{2}y_{\tiles,n}/(1-m_{\tiles,n})$. Both $\tilde A_{\tiles,n}$ and $\tilde c_{\tiles,n}$ can be expressed in terms of $a_{\tiles,n}$ and $b_{\tiles,n}$, and thus of $y_{\tiles,n}$ and $m_{\tiles,n}$:
\begin{align}
\tilde A_{\tiles,n} &= a_{\tiles,n}b_{\tiles,n} \int_0^1 \eta(1-\eta)\, \diff \eta = \frac{a_{\tiles,n}b_{\tiles,n}} 6 = \frac{y_{\tiles,n}^2}{3(1-m_{\tiles,n}^2)}, \\
\tilde c_{\tiles,n} &= \sqrt{a_{\tiles,n}^2+b_{\tiles,n}^2} = \frac{2 y_{\tiles,n} \sqrt{1+m_{\tiles,n}^2}}{1-m_{\tiles,n}^2}.
\end{align}
Substituting into \eqref{eq: P i j asympt 3 bis},
\begin{equation}
\label{eq: P i j asympt 4}
P_{\tiles,n}
= \frac{4\sqrt{2} \sqrt{1+m_{\tiles,n}^2}} {3\pi(1-m_{\tiles,n}^2)^2} y_{\tiles,n}^3 + E_{\tiles, n}.
\end{equation}
The slope $m_{\tiles,n}$ is bounded by \eqref{eq: asympt slope bound}, which implies that $m_{\tiles,n}^2 \leq 2/(\tiles-2)$. Using this inequality together with \eqref{eq: sqrt asympt}, the term depending on $m_{\tiles,n}^2$ in \eqref{eq: P i j asympt 4} can be bounded as
\begin{equation}
\label{eq: P i j asympt 4 bis}
\frac{\sqrt{1+m_{\tiles, n}^2}} {(1-m_{\tiles, n}^2)^2} \leq \frac{(\tiles-2)^2}{(\tiles-4)^2} \left(1+\frac 1 {\tiles-2}\right) = 1 + \frac{5\tiles - 14}{(\tiles-4)^2}.
\end{equation}
On the other hand, $y_{\tiles,n}$ can be bounded from \eqref{eq: h bound} as $y_{\tiles,n} \leq h_\tiles \leq 1$. Using this inequality as well as \eqref{eq: P i j asympt 4 bis} into \eqref{eq: P i j asympt 4},
\begin{equation}
\label{eq: P i j asympt 5}
P_{\tiles,n} = \frac{4\sqrt{2}} {3\pi} y_{\tiles,n}^3 + E_{\tiles,n} + E'_{\tiles,n}
\end{equation}
with $|E'_{\tiles,n}| < e'(t)$, where $e'(t)$ tends to $0$ as $t \rightarrow \infty$.

Let $\funcircesc_\tiles$ be defined as a scaled version of $\funcirc$, where the variable $x \in [-w_\tiles, w_\tiles]$ is replaced by $\zeta = x/w_\tiles \in [-1,1]$:
\begin{equation}
...
\end{equation}
As $\tiles \rightarrow \infty$, the sequence of functions $\funcircesc_\tiles$ converges pointwise to
\begin{equation}
\label{eq: funcircesc}
\funcircesc(\zeta) = \lim_{\tiles \rightarrow \infty} \funcircesc_\tiles(\zeta) = \frac{1 -\zeta^2}{\sqrt{2}}.
% \frac{\sqrt{2}}{2}(1 -\zeta^2)
\end{equation}
Furthermore, the convergence is uniform. To show this, it is easier to consider the subsequences of $\tiles$ odd and even separately. For $\tiles$ odd, substituting $\len_\tiles$, $w_\tiles$ and $h_\tiles$ given by \eqref{eq: len tiles}, \eqref{eq: w} and \eqref{eq: h tiles} into \eqref{eq: funcircest tiles 1},
\begin{equation}
...
\end{equation}
This implies that the subsequence $\funcircesc_\tiles$ with $\tiles$ odd is monotone non-increasing. Since the limit function $\funcircesc$ is continuous, by Dini's theorem \cite[section~1.1]{Hirsch99} the convergence of this subsequence is uniform. On the other hand, for $\tiles$ even
\begin{equation}
\label{eq: funcircesc, even 1}
\funcircesc_\tiles(\zeta) =
% -\frac{\tiles-3}{\sqrt{2}} + \sqrt{\frac{(\tiles-2)^2}{2} - \left(\tiles-\frac 5 2\right)\zeta^2}
...
\frac{\partial \funcircesc_\tiles(\zeta)}{\partial \tiles} = \frac 1 {\sqrt{2}} \left(
-1 + \frac{\tiles-2-\zeta^2}{\sqrt{(\tiles-2)^2 - 2(\tiles-2)\zeta^2 + \zeta^2}} \right) \leq 0.
\end{equation}
Thus the subsequence $\funcircesc_\tiles$ with $\tiles$ even is also monotone non-increasing, and therefore converges uniformly. Consequently, the full sequence $\funcircesc_\tiles$ converges to $\funcircesc$ uniformly on $[-1,1]$. This means that
\begin{equation}
\label{eq: funcircest tiles 4}
y_{\tiles,n} = \funcirc(x_{\tiles,n}) = \funcircesc_\tiles\left( \frac{x_{\tiles,n}} {w_\tiles} \right) = \funcircesc\left( \frac{x_{\tiles,n}} {w_\tiles} \right) + E''_{\tiles,n}
\end{equation}
with $|E''_{\tiles,n}| < e''(t)$, where $e''(\tiles)$ tends to $0$ as $\tiles \rightarrow \infty$.

Combining \eqref{eq: sum p t n}, \eqref{eq: P i j asympt 5} and \eqref{eq: funcircest tiles 4},
\begin{equation}
\label{eq: sum p t n aprox 1}
\tiles \probmax(\len_\tiles) = \sum_{n=1}^N P_{\tiles,n} = \frac{4 \sqrt{2}} {3\pi} \sum_{n=1}^N \funcircesc^3 \left( \frac{x_{\tiles,n}}{w_\tiles} \right) + \sum_{n=1}^N \left(E_{\tiles,n} + E'_{\tiles,n} + E''_{\tiles,n} \right),
\end{equation}
which can be rewritten as
\begin{equation}
\label{eq: sum p t n aprox 2}
\sqrt{\tiles} \probmax(\len_\tiles) = \frac{4 w} {3\pi\sqrt{\tiles}} \frac {\sqrt{2}} {w} \sum_{n=1}^N \funcircesc^3 \left( \frac{x_{\tiles,n}}{w_\tiles} \right) +
\frac{N} {\sqrt{t}} \frac 1 N \sum_{n=1}^N (E_{\tiles,n} + E'_{\tiles,n} + E''_{\tiles,n}).
\end{equation}
Noting that the values $\zeta_{\tiles,n}$ are equispaced from $-1$ to $1$ with step $\sqrt{2}/w$, the term $\sqrt{2}/w \sum_{n=1}^N \funcircesc^3 ( x_{\tiles,n}/w_\tiles)$ in \eqref{eq: sum p t n aprox 2} can be interpreted as a Riemmann sum that approximates the integral of $\funcircesc^3(\zeta)$ over $[-1,1]$. Since this function is continuous the sum indeed converges to the integral \cite[section~7.2]{Abbott15}. On the other hand,
\begin{equation}
\left| \frac 1 N \sum_{n=1}^N \left( E_{\tiles,n} + E'_{\tiles,n} + E''_{\tiles,n} \right) \right| < e(\tiles)+e'(\tiles)+e''(\tiles)
\end{equation}
and the right-hand side tends to $0$ as $\tiles \rightarrow \infty$. The terms $w/\sqrt{\tiles}$ and $N/\sqrt{\tiles}$ in \eqref{eq: sum p t n aprox 2} tend to $1$ and $\sqrt{2}$ respectively as $\tiles \rightarrow \infty$. Therefore, taking limits and substituting \eqref{eq: funcircesc},
\begin{equation}
