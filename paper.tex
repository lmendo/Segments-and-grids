...
\begin{abstract}
The relationship between the length of a line segment and the number of tiles it visits on a rectangular grid is addressed in this paper. For a segment with deterministic but arbitrary position and orientation, the maximum number of visited tiles given its length and the infimum length for a desired number of visited tiles are characterized. For a segment with random position and orientation, the average number of visited tiles and the probability of visiting the maximum number of tiles are studied. These questions are related to Buffon's needle problem and its extension by Laplace.

\emph{Keywords:} Discrete geometry, Geometric probability, Rectangular lattice, Buffon's needle problem.

\emph{MSC2020:} 52C99, 60D05. % ***Alex: check if there's a better one
