...
Consider $a=b=1$ first. This is illustrated in Figure~\ref{fig: ijLS_1}. Note that in this and in the next figures the axes represent $ia$ and $jb$ (not $i$ and $j$). It is convenient, however, to refer to the point $(ia,jb)$ by its normalized coordinates $(i,j)$. In this graph, each dashed diagonal line contains pairs $(i,j)$ with the same $i+j-1$; and the lower bound \eqref{eq: len > sqrt} corresponds to an arc centered at normalized coordinates $(2,2)$.
...
As a second example, consider $a=1.35$, $b=1$. This is depicted in Figure~\ref{fig: ijLS_1p35}. Again, the optimal pair $(i,j)$ for each diagonal is that closest to $(2,2)$; but in this case the $i,j$ coordinates of these pairs do not follow a rule as simple as in the previous example.
...
% ***This figure and Figure~\ref{fig: ijLS_1p35_detail} label the points as (i,j) instead of (ia,jb). Clarify in the text, or change the figures ***Maybe use the concept of normalized coordinates
\begin{figure}%
\centering%
...
***Graph of asymptotic slopes: $\sqrt(1/a^2+1/b^2)$ for $\funt$ and $2/\pi(1/a+1/b)$. It may be interesting to consider the ratio of asymptotic slopes, as a function of $a/b$. It is maximum for $a=b$, and equals $2\sqrt 2/\pi = 0.9003$. It tends to $2/\pi$ for $a/b \rightarrow \infty$ and for $a/b \rightarrow 0$.
